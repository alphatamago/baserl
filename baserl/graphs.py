...
def heatmap_value_function(value_function,
    ...
    """
    mapping_key_func extracts the (x,y) coordinates for 2-D state space (in case the state is more complex)
    make_default_key_func mapx (x,y) to a default state value (in case the state is more complex)
    """
    xs = set()
    ys = set()
